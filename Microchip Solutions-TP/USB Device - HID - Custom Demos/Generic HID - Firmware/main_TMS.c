...
void Send_Serial(unsigned char *SendBuffer, unsigned int len);
...
int result=0;
TRISD = 0b11110000;; // Lower 4 bits are used for LED.
TRISB = 1;
TRISA = 0b00000011;
PORTD = 0x01; // Initial value for PORTD.
PORTAbits.RA2 = 0;
...
//    #endif
	...
	if(DataRdyUSART())
	{
		rx_buffer[recv_cnt] = ReadUSART(); // Read the byte.
		recv_cnt++;
			...
			if(rx_buffer[2] == 0x80) //
			{
					if (Switch2IsPressed()) //test tube 1
					{
					tx_buffer[0] = 0xAB;
					tx_buffer[1] = 0x01;
					tx_buffer[2] = 0x80;
					tx_buffer[3] = 0x02;
					tx_buffer[4] = 0xFF - tx_buffer[0]- tx_buffer[1] - tx_buffer[2] - tx_buffer[3];
					
					PORTAbits.RA2 = 1;
					Send_Serial(&tx_buffer[0],5);
					PORTAbits.RA2 = 0;
	
					mLED_3_On();
					}
					else if (Switch3IsPressed()) //test tube 0
					{
					tx_buffer[0] = 0xAB;
					tx_buffer[1] = 0x01;
					tx_buffer[2] = 0x80;
					...
					else
					{
						...
						result = (ADRESH*256+ADRESL)/4;
						
						tx_buffer[0] = 0xAB;
						tx_buffer[1] = 0x01; //PC address
						tx_buffer[2] = 0x82;
						tx_buffer[3] = result;
			...
			if(rx_buffer[2] == 0x81) //cancel alarm
				{
				mLED_3_Off();
				mLED_4_Off();
...
{  
	...
	/***** START: Temperature Monitoring System application started here *****/

	if(!HIDRxHandleBusy(USBOutHandle))			//Check if data was received from the host.
    { 
		if (ReceivedDataBuffer[2] == 0x41)		// Cmd = 0x41 is temperature checking command sent from the host.
        {										
			// Part 1: Write your codes here
			if (Switch2IsPressed())
			{
				ToSendDataBuffer[0] = 0xAB;
				ToSendDataBuffer[1] = 0x01; //PC address
				ToSendDataBuffer[2] = 0x41; //temp exceeded limit
				ToSendDataBuffer[3] = 0x02;
				ToSendDataBuffer[4] = 0xFF - ToSendDataBuffer[1]-ToSendDataBuffer[2]-ToSendDataBuffer[3];
				mLED_3_On();
			}
			else if (Switch3IsPressed())
			{
				ToSendDataBuffer[0] = 0xAB;
				ToSendDataBuffer[1] = 0x01; //PC address
				ToSendDataBuffer[2] = 0x41;
				ToSendDataBuffer[3] = 0x03;
				ToSendDataBuffer[4] = 0xFF - ToSendDataBuffer[1]-ToSendDataBuffer[2]-ToSendDataBuffer[3];
				mLED_4_On();
			}
			...
			else
			{
				...
				ToSendDataBuffer[0] = 0xAB;
				ToSendDataBuffer[1] = 0x01; //PC address
				ToSendDataBuffer[2] = 0x42;
				ToSendDataBuffer[3] = result;
				ToSendDataBuffer[4] = 0xFF - ToSendDataBuffer[1]-ToSendDataBuffer[2]-ToSendDataBuffer[3];
			}
		...
		if (ReceivedDataBuffer[2] == 0x44)		// Cmd = 0x44 is Alarm Cancel command sent from the host.
        {
			// Part 2: Write your codes here
			mLED_3_Off();
			mLED_4_Off();
			
			ToSendDataBuffer[0] = 0xAB;
			ToSendDataBuffer[1] = 0x01; //PC address
			ToSendDataBuffer[2] = 0x44;
	...
	/***** END: Temperature Monitoring System application ended here *****/
}//end ProcessIO
