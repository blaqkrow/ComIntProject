...
void USBCBSuspend(void)
{
	...
	//things to not work as intended.	
    	...
    	// Go to sleep - micro power mode
        TRISC = 0xFF;           //Disable the LEDs (a couple mA of current)
        //Cutting J3 will remove the POT from the circuit reducing the 
        //  overall current by .5mA.
        PIR2bits.USBIF = 0;     //Get rid of any pending USB interrupts
    	USBActivityIE = 1;      //Enable the activity interrupt
    	PIE2bits.USBIE = 1;     //Enable the USB interrupt
    	INTCONbits.PEIE = 1;    //Enable interrupts
    	Sleep();                //sleep
    	PIE2bits.USBIE = 0;     //disable the usb interrupt
    	INTCONbits.PEIE = 0;    //disable interrupts
        mInitAllLEDs();         //enable the LEDs again
    #endif
